function [theta, dist] = bayes_toad_abc_mmd_log(Y,simArgs,sumArgs,tol,M,cov_rw,prior,w)
% ABC using MMD distance for toad example.  Log transform is applied to the data.
nan_pos = isnan(Y);
...
for i = 1:M
    ...
    dist_prop = sum(w.*[MMD_dist(log(x{1,2}),log(y{1,2})) MMD_dist(log(x{2,2}),log(y{2,2})) MMD_dist(log(x{3,2}),log(y{3,2})) MMD_dist(log(x{4,2}),log(y{4,2})) abs(x{1,1} - y{1,1}) abs(x{2,1} - y{2,1}) abs(x{3,1} - y{3,1}) abs(x{4,1} - y{4,1})]);
