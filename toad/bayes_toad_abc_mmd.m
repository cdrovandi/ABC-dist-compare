function [theta, dist] = bayes_toad_abc_mmd(Y,simArgs,sumArgs,tol,M,cov_rw,prior,w)
% ABC using MMD distance for toad example
nan_pos = isnan(Y);
...
for i = 1:M
    ...
    dist_prop = sum(w.*[MMD_dist(x{1,2},y{1,2}) MMD_dist(x{2,2},y{2,2}) MMD_dist(x{3,2},y{3,2}) MMD_dist(x{4,2},y{4,2}) abs(x{1,1} - y{1,1}) abs(x{2,1} - y{2,1}) abs(x{3,1} - y{3,1}) abs(x{4,1} - y{4,1})]);
