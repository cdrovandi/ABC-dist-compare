function [theta, dist] = bayes_toad_abc_cvm(Y,simArgs,sumArgs,tol,M,cov_rw,prior,w)
% ABC using CvM distance for toad example
nan_pos = isnan(Y);
...
for i = 1:M
    ...
    dist_prop = sum(w.*[cvm_stat_M(x{1,2},y{1,2},2) cvm_stat_M(x{2,2},y{2,2},2) cvm_stat_M(x{3,2},y{3,2},2) cvm_stat_M(x{4,2},y{4,2},2) abs(x{1,1} - y{1,1}) abs(x{2,1} - y{2,1}) abs(x{3,1} - y{3,1}) abs(x{4,1} - y{4,1})]);
